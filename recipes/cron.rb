...
cron "Graylog2 send stream alarms" do
  ...
  command "cd #{node["graylog2"]["basedir"]}/web && RAILS_ENV=production bundle exec rake streamalarms:send 1> /dev/null"
end
...
cron "Graylog2 send stream subscriptions" do
  ...
  command "cd #{node["graylog2"]["basedir"]}/web && RAILS_ENV=production bundle exec rake subscriptions:send 1> /dev/null"
end
